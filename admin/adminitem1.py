...
@st.dialog(title="Description", width="large")
...
with tab3:
    ...
    with image_undst:
        ...
        if st.button("Description_image"):
            desc = """In this demo, you will be presented with a scene (e.g., a living room) and will use the Gemini 1.5 Pro model to perform visual understanding. You will see how Gemini 1.0 can be used to recommend an item (e.g., a chair) from a list of furniture options as input. You can use Gemini 1.5 Pro to recommend a chair that would complement the given scene and will be provided with its rationale for such selections from the provided list.
                        """
            desc = desc + "\nExpected Result: Recommend a chair that would complement the given image of a living room."
    ...
    with screens_undst:
        ...
        if st.button("Description_screen"):
            desc = "Equipped with the ability to extract information from visual elements on screens, Gemini 1.5 Pro can analyze screenshots, icons, and layouts to provide a holistic understanding of the depicted scene."
            desc = desc + "\n\nExpected Result: Provide instructions for resetting the clock on this appliance in English"
...
"""
    ...
    with diagrams_undst:
        ...
        if st.button("Description_er"):
            desc = "Gemini 1.5 Pro multimodal capabilities empower it to comprehend diagrams and take actionable steps, such as optimization or code generation. The following example demonstrates how Gemini 1.0 can decipher an Entity Relationship (ER) diagram."
            desc = desc + "\n\nExpected Result: Document the entities and relationships in this ER diagram."
    ...
    with recommendations:
        ...
        if st.button("Description_diag"):
            desc = """Gemini 1.5 Pro is capable of image comparison and providing recommendations. This may be useful in industries like e-commerce and retail.
                    Below is an example of choosing which pair of glasses would be better suited to various face types:"""
            desc = desc + "\n\n Expected Result: Suggest which glasses type is better for the {face_type} face shape"
    ...
    with sim_diff:
        ...
        if st.button("Description_math"):
            desc = "Gemini 1.5 Pro can also recognize math formulas and equations and extract specific information from them. This capability is particularly useful for generating explanations for math problems, as shown below."
            desc = desc + "\n" + """
                        Expected Result: Ask questions about the math equation as follows:
                            - Extract the formula.
